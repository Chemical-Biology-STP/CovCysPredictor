...
# Automatically generated on Thu Jul 21 18:57:00 2022
...
# VERSION ensprop.svl    33618.2020.10.21.12.57.36  $MOE/svl/biopolymer.svl

# Usage:
# Protein Properties run.sh
...
task_name='MOE(awooner1)(yz92jq)propsamp'
task_qsys='sh'
...
echo
echo "MOEBATCH -stdc -run "$0" "$@""
MOEBATCH -run "$0" "$@"	# will exit 0 or 1
exit $?			# safety
...
//
//	File: run.sh
//	Date: Thu Jul 21 18:57:00 2022
//	Creator: EnsembleProteinProperties in MOE (2020.09)
//
// LICENSE:
//
// COPYRIGHT (C) 2020 CHEMICAL COMPUTING GROUP ULC ("CCG").
...
// INPUT FILES:
//
// Input structures: structures.mdb
//
// OUTPUT FILES:
//
// Conformation MDB file: structures_ens.mdb
//
// USAGE:
//    sh run.sh <submit options> -- [-nthreads <N> | -force | -continue]
//        -nthreads: use N threads per CPU (override default setting)
//        -force: overwrite any existing output file
//        -continue: use existing output file to continue interrupted run

const OPT = [
    ipH:	6.4,
    fpH:	8.4,
    nconf:	100,
    dbmol:	'receptor_prepared',
    filename:	'structures_ens.mdb',
    selonly:	0,
    no_prep:	0,
    merge:	0,
    settings:	[['bb_resfilt','ASN;ASP'],['amide_n',0],['met_s',1],['solv_cut',5],['probe',1.8],['hcutoff',0.09],['hminarea',50],['qcutoff',40],['qminarea',40],['salt','NaCl'],['viscosity',0.89],['dielectric',78],['sc_resfilt',''],['T',300],['salt_conc',0.1]],
    props:	['res_exp','res_ASA','res_charge','propKa'],
    verbose:	1,
    nthreads:	1,
    quiet:	1
];
const MDB = 'structures.mdb';

function pro_StochasticTitration;
local function main []
    pot_Load '$MOE/lib/Amber10EHT.ff.gz';
    pot_Setup [['strEnable',1],['angEnable',1],['stbEnable',1],['oopEnable',1],['torEnable',1],['vdwEnable',1],['eleEnable',1],['solEnable',0],['resEnable',1],['strWeight',1],['angWeight',1],['stbWeight',1],['oopWeight',1],['torWeight',1],['vdwWeight',1],['eleWeight',1],['solWeight',1],['resWeight',1],['cutoffEnable',1],['cutoffOn',8],['cutoffOff',10],['eleDist',2],['vdwScale14',0.5],['vdwBuffer1',0],['vdwBuffer2',0],['eleScale14',0.8333333333],['eleDielectric',1],['eleBuffer',0],['solDielectric',80],['solDielectricOffset',0],['state0',1],['state1',0],['state2',1],['threadCount',0]];

    pro_StochasticTitration [MDB, OPT];
endfunction
